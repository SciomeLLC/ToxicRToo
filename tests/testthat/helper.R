build_ma_dataset_2 <- function() {
  ...
  mean <- ToxicRToo:::.cont_hill_f(as.numeric(hill[1, ]), doses)
  set.seed(2020)
  y <- actuar::rinvgauss(length(mean), mean, 45528.14)
  list(doses = doses, y = y)
}
...
generate_validation_code_single <- function(c) {
  ...
  if ("fitted_model" %in% names(c)) {
    cat(
      "validate_model(c, ",
      paste0("\"", c$fitted_model$full_model, "\""),
      ", ",
      paste(list(c$fitted_model$parameters), sep = ", "),
      ", ",
      paste(list(c$bmd), sep = ", "),
      ")\n"
    )
  } else {
    cat(
...
generate_validation_code <- function(AA) {
  ...
  for (i in 1:(length(AA) - 3)) {
    cat(
      "validate_model(",
      paste0("AA$Individual_Model_", i),
      ", ",
      paste0("\"", AA[[i]]$full_model, "\""),
      ", ",
      paste(list(AA[[i]]$parameters), sep = ", "),
      ", ",
      paste(list(AA[[i]]$bmd), sep = ", "),
      ")\n"
    )
  }
}
...
build_ma_dataset <- function() {
  ...
  mean <- ToxicRToo:::.cont_hill_f(as.numeric(hill[6, ]), doses)
...
build_model_list <- function(y) {
  ...
  model_list <- list()
  for (i in seq_len(nrow(model_listA))) {
    t_prior <- ToxicRToo:::.bayesian_prior_continuous_default(
      model_listA$model_list[i],
      model_listA$distribution_list[i]
    )
    ...
    model_list[[i]] <- create_continuous_prior(
      t_prior,
      model_listA$model_list[i],
      model_listA$distribution_list[i]
    )
