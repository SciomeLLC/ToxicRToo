...
test_that("Plots", {
  ...
  c <- single_continuous_fit(M2[, 1, drop = FALSE], M2[, 2:4],
    BMR_TYPE = "sd", BMR = 1, ewald = TRUE,
    distribution = "normal", fit_type = "mle", model_type = "polynomial",
    degree = 4
  )
  mle_plot <- plot(c)
  ...
  expect_identical(
    ...
    "Model: Polynomial Distribution: Normal,  Fit Type: Maximized"
  )
  c <- single_continuous_fit(M2[, 1, drop = FALSE], M2[, 2:4],
    BMR_TYPE = "sd", BMR = 1, ewald = TRUE,
    distribution = "normal", fit_type = "mle", model_type = "hill", degree = 4
  ...
  expect_identical(
    mle_plot$labels$title,
    "Model: Hill Distribution: Normal,  Fit Type: Maximized"
  )
})
