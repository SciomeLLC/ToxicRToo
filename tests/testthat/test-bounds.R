...
test_that("Laplace gamma-efsa bounds are respected", {
  ...
  Y <- cont_data[, 2:4]
  test_new2 <- single_continuous_fit(
    cont_data[, 1],
    Y,
    model_type = "gamma-efsa",
    fit_type = "laplace",
...
test_that("Posterior Probabilities dont differ", {
  ...
  cont_data[, 3] <- rep(5, 5)
  Y <- cont_data[, 2:4]
  suppressWarnings(fit <- ma_continuous_fit(
    cont_data[, 1],
    Y,
    alpha = 0.025,
    fit_type = "mcmc"
  ...
  probability_diff <- sum(
    abs(fit@posterior_probs - fit1@posterior_probs),
    na.rm = TRUE
  )
