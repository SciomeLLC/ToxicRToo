...
positive_dataset_genes <- c(
  "GDF15_33113", "A2M_7932", "STAC3_9953", "ANXA7_8051",
...
intermediate_dataset_genes <- c(
  "ALDH6A1_8027", "ATF3_8095", "CXCL1_8407",
...
build_positive_response_dataset <- function(target_gene) {
  ...
  gene_mean <- unlist(
    mean_data[grep(target_gene, mean_data[, 1]), -1],
    use.names = FALSE
  )
  gene_stdev <- unlist(
    stdev_data[grep(target_gene, stdev_data[, 1]), -1],
    use.names = FALSE
  )
...
build_intermediate_response_dataset <- function(target_gene) {
  mean_data <- read.csv(
    "datasets/intermediate_control_mean.txt",
    header = FALSE
  )
  stdev_data <- read.csv(
    "datasets/intermediate_Control_stdev.txt",
    header = FALSE
  )
  gene_mean <- unlist(
    mean_data[grep(target_gene, mean_data[, 1]), -1],
...
run_model <- function(M2, model_type, fit_type, distribution) {
  model_res <- single_continuous_fit(M2[, 1, drop = FALSE], M2[, 2:4],
    BMR_TYPE = "sd", BMR = 1, ewald = TRUE,
    distribution = distribution, fit_type = fit_type,
    model_type = model_type, degree = 4
  )
...
model_runner <- function(gene_list, gene_dataset_creator) {
  results <- data.frame(
    model = character(), distribution = character(),
  ...
  for (gene in gene_list) {
    ...
    for (model_type in c(
      ...
      for (fit_type in c("laplace", "mle", "mcmc")) {
        for (distribution in c("normal", "normal-ncv", "lognormal")) {
          if (
            model_type %in% c("hill", "power", "polynomial") &&
              distribution == "lognormal") {
            # Log-Normal/Hill specification is not presently available.
            # Power-Log-normal models are not allowed. Please choose normal
            # or normal non-constant variance.
            # Polynomial-Log-normal models are not allowed. Please choose
          ...
          if (model_type %in% c(
            ...
            # Aerts models are currently not supported with the frequentist
            # approach
            next
          }
          result <- single_continuous_fit(M2[, 1, drop = FALSE], M2[, 2:4],
            BMR_TYPE = "sd", BMR = 1, ewald = TRUE,
            distribution = distribution, fit_type = fit_type,
            model_type = model_type, degree = 4
          )
          ...
          if (num_parameters == 4) {
            df <- data.frame(
              ...
              parameter2 = result$parameters[2],
              parameter3 = result$parameters[3],
              parameter4 = result$parameters[4],
              parameter5 = NA,
              parameter6 = NA,
              parameter7 = NA,
              bmd = bmds[1],
            ...
            df <- data.frame(
              ...
              parameter3 = result$parameters[3],
              parameter4 = result$parameters[4],
              parameter5 = result$parameters[5],
              parameter6 = NA,
              parameter7 = NA,
              bmd = bmds[1],
              bmdl = bmds[2],
            ...
            df <- data.frame(
              ...
              parameter4 = result$parameters[4],
              parameter5 = result$parameters[5],
              parameter6 = result$parameters[6],
              parameter7 = NA,
              bmd = bmds[1],
              bmdl = bmds[2],
              bmdu = bmds[3]
            ...
            df <- data.frame(
              ...
              parameter5 = result$parameters[5],
              parameter6 = result$parameters[6],
              parameter7 = result$parameters[7],
              bmd = bmds[1],
              bmdl = bmds[2],
              bmdu = bmds[3]
            )
    ...
    write.csv(
      results,
      file = paste0("positive_control_", gene, "_results.csv"),
      row.names = FALSE
    )
    results <- data.frame(
      model = character(), distribution = character(),
